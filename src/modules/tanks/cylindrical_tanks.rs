...
pub fn compute_tank_height(
    ...
    {
        if max_wet_mass <= payload_mass_kg + engine_mass_total {
    ...
    let correction_factor_total = (K * diameter * diameter * diameter * n) * 0.001;
    let ellipsoid_volume_total = n * ellipsoid_volume(r, r, r / 2.0);
    ...
    */
    let numerator_1 = max_wet_mass - payload_mass_kg - engine_mass_total;
...
pub fn compute_tank_height_for_delta_v(
    ...
    let num_1 = (payload_mass_kg + engine_mass) * (1.0 - e0);
...
mod tests {
    ...
    #[test]
    fn dry_mass_tests_v3() {
        let h = 2.822;
        let d = 0.3;
        let fuselages = CylindricalTank::init_fuselage_types();
        let fuselage = fuselages.hp_fuselages.get("HP Steel Fuselage").unwrap();
        let v = tank_volume(d, h);
        let dry_mass = v * fuselage.density * (1.0 - fuselage.utilization);
        assert!((dry_mass - 56.8).abs() < 0.1);
    }

    #[test]
    fn twr_wet_mass_dry_mass_confirmation() {
        let payload_mass = 0.087;
        let payload_mass_kg = payload_mass * 1000.0;
        ...
        .unwrap();
        assert!((h - 2.821).abs() < 0.1, "h was off, {}", h);
    ...
    fn tank_height_tests() {
        ...
        let (h, _twr, _wet_mass, _dry_mass, _volume) = compute_tank_height(
            target_twr,
            &engines[0],
            &fuselage,
            payload_mass_kg,
